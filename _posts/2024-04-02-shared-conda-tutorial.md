...
First, we create a shared conda installation folder that is read-only for all regular users. Shared conda environments therein, which are available to all users, must be created by a sysadmin (or by a user with write-privileges for the conda folder). 
